...
import { newUser } from '../../functions/types/types';
...
import { SignUpSchema } from '../../functions/Schema';
import {
  useCreateAccount,
  useSignInAccount,
...
const SignupForm = () => {
  ...
  const { mutateAsync: signinMutate, isPending: signing } = useCreateAccount();
  const { mutateAsync: loginMutate } = useSignInAccount();

  async function submitHandler(data: newUser) {
    if (await isEmailAvailable(data.email)) {
      alert('email is already exist');
    } else {
      const user = await signinMutate(data);
      if (!user) {
            ...
            >
              ...
              <div className="form-group">
                ...
                <div className="password-wrapper">
                  ...
                  <div
                    className="password-btn"
                    onClick={() => setConfirmPasswordVisible((state) => !state)}
                  >
              ...
              <button
                ...
                className="py-3 border-solid border-[1px]  w-full rounded-lg  text-white bg-red-600  disabled:cursor-not-allowed hover:bg-white hover:text-red-600 duration-200 "
              >
                {signing ? 'Signing up....' : 'Sign Up'}
              </button>
              <p className="w-full text-center mt-2 text-[15px]">
                Already have an account?
                <Link className="text-blue-800" to="/signin">
